...
export class ProfileService implements OnInit {
  ...
  constructor(
   ...
   ngOnInit() {
         ...
         if (token) { this.getUserSource(token); } else { return null; }
   }

   getLocation() {  this.http.post('/api/user/location', { location: '' }).subscribe(
         (response: any) => this.locSource.next(response.country),
         catchError( error => throwError(error) )
      );
   }

   checkAuthToken(authToken) {  return this.http.post('/api/user/register/authenticate', {authToken}).subscribe(
         (response: User) => {
   ...
   getUserSource(jwtToken): void {  if (jwtToken) {
   ...
   register(formValue) {  this.http.post('/api/user/register', formValue)
         .subscribe(
            (response: User) => {
   ...
   getLoggedStatus() {  const token = localStorage.getItem('token');
                        const helper = new JwtHelperService();
                        const expired = helper.isTokenExpired(token);
                        const user = helper.decodeToken(token);
                        if (!expired && user) { return {expired, user}; } else {
         this.snackBar.open('You are not authorized to acces this page, please Login', 'X', {duration: 10000, panelClass: 'red-theme'});
         this.router.navigate(['']);
      }
   }        //

   getCities(value) {  return this.http.post('/api/user/profile/autocomplete/cities', value).pipe(
         catchError(error => throwError(error))
      );
   }

   getStates(value) {  return this.http.post('/api/user/profile/autocomplete/states', value).pipe(
         catchError(error => throwError(error))
      );
   }
   ...
   updateUser(userData) {  return this.http.post('/api/user/profile/update', userData, httpOptions).pipe(
         catchError(error => throwError(error))
      );
   }

   updatePhoneCode(userData) {  return this.http.post('/api/user/profile/update/autocomplete/country', userData, httpOptions).pipe(
         catchError(error => throwError(error))
      );
   }
